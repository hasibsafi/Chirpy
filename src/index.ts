...
app.post("/api/chirps", express.json(), ErrorHandler, async (req: Request, res: Response, next: NextFunction) => {
   ...
   try {
    const chirp = await createChirp({ body: result.cleanedBody! ?? body, userId });
...
app.delete("/api/chirps/:id", express.json(), async (req: Request, res: Response) => {
  let {id} = req.params;
  if (Array.isArray(id)) {
    id = id[0];
  }
  ...
  if(chirp.userId !== userId) {
    return res.status(403).json({ error: "You are not authorized to delete this chirp" });
  }
...
app.post("/api/users", express.json(), ErrorHandler, async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
       ...
       res.status(201).json({id: user.id, email: user.email, createdAt: user.createdAt, updatedAt: user.updatedAt, isChirpyRed: user.isChirpyRed});
...
app.post("/api/login", express.json(), ErrorHandler, async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const ONE_HOUR = 3600;
    const expiresIn = typeof expiresInSeconds === "number"
    ...
    res.status(200).json({id: user.id, email: user.email, createdAt: user.createdAt, updatedAt: user.updatedAt, token: accessToken, refreshToken, isChirpyRed: user.isChirpyRed});
  }catch(err) {}
});


// Refresh Token Endpoint
app.post("/api/refresh", ErrorHandler, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const ONE_HOUR = 3600;
    const accessToken = makeJWT(tokenRecord.id, ONE_HOUR, config.api.jwtSecret);
...
app.post("/api/revoke", ErrorHandler, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const refreshToken = getBearerToken(req);
...
app.put("/api/users", express.json(), ErrorHandler, async (req: Request, res: Response, next: NextFunction) => {
  const {email, password} = req.body;
  if (!email || !password) {
    throw new BadRequestError("Email and password are required");
  }
  ...
  try {
    ...
    res.status(200).json({id: user.id, email: user.email, createdAt: user.createdAt, updatedAt: user.updatedAt, isChirpyRed: user.isChirpyRed});
  }catch(err){
    throw new BadRequestError("something went wrong")
    next(err);
  }
...
app.post("/api/polka/webhooks", express.json(), ErrorHandler, async (req: Request, res: Response, next: NextFunction) => {
  try {
