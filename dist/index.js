...
app.post("/api/chirps", express.json(), ErrorHandler, async (req, res, next) => {
    ...
    try {
        ...
        if (!chirp)
            return res.status(500).json({ error: "Chirp not created" });
...
app.get("/api/chirps/:id", express.json(), async (req, res) => {
    let { id } = req.params;
    if (Array.isArray(id)) {
...
app.post("/api/users", express.json(), ErrorHandler, async (req, res, next) => {
...
// Login endpoint
app.post("/api/login", express.json(), ErrorHandler, async (req, res, next) => {
    const { email, password } = req.body;
    if (!email || !password) {
        throw new BadRequestError("Email and password are required");
    }
